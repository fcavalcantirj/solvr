...
describe('sitemap', () => {
  ...
  it('returns static pages combined with dynamic URLs from API', async () => {
    vi.mocked(api.getSitemapUrls).mockResolvedValue({
      data: {
        ...
        blog_posts: [
          { slug: 'hello-world', updated_at: '2026-02-04T00:00:00Z' },
        ],
      },
    });
    ...
    // 12 static + 3 posts + 1 agent + 1 user + 1 blog = 18
    ...
    expect(urls).toContain('https://solvr.dev/blog');
    // Dynamic pages
    ...
    expect(urls).toContain('https://solvr.dev/blog/hello-world');
  });
  ...
  it('sets correct priorities for each type', async () => {
    vi.mocked(api.getSitemapUrls).mockResolvedValue({
      data: {
        ...
        blog_posts: [{ slug: 'test-post', updated_at: '2026-02-01T00:00:00Z' }],
      },
    });
    ...
    const blogStatic = result.find((e) => e.url === 'https://solvr.dev/blog');
    expect(blogStatic?.priority).toBe(0.8);

    const post = result.find((e) => e.url === 'https://solvr.dev/problems/p1');
    expect(post?.priority).toBe(0.7);

    const blogPost = result.find((e) => e.url === 'https://solvr.dev/blog/test-post');
  ...
  it('handles missing blog_posts gracefully for backwards compatibility', async () => {
    vi.mocked(api.getSitemapUrls).mockResolvedValue({
      data: {
        posts: [{ id: 'p1', type: 'problem', updated_at: '2026-02-01T00:00:00Z' }],
        agents: [],
    ...
    // 12 static + 1 post + 0 blog = 13
    expect(result).toHaveLength(13);
    const urls = result.map((entry) => entry.url);
    expect(urls).toContain('https://solvr.dev/blog');
    expect(urls).not.toContain('https://solvr.dev/blog/undefined');
  });

  it('sets weekly changeFrequency and correct lastModified for blog posts', async () => {
    vi.mocked(api.getSitemapUrls).mockResolvedValue({
      data: {
        posts: [],
        agents: [],
        users: [],
        blog_posts: [{ slug: 'my-post', updated_at: '2026-02-15T10:00:00Z' }],
      },
    });

    const result = await sitemap();

    const blogEntry = result.find((e) => e.url === 'https://solvr.dev/blog/my-post');
    expect(blogEntry).toBeDefined();
    expect(blogEntry?.changeFrequency).toBe('weekly');
    expect(blogEntry?.lastModified).toBe('2026-02-15T10:00:00Z');
    expect(blogEntry?.priority).toBe(0.7);
  });
});
