...
import { transformIdea } from "@/hooks/use-ideas";
...
export function IdeasPageClient({ initialPosts }: IdeasPageClientProps) {
  const router = useRouter();
  const { isAuthenticated } = useAuth();
  const { stats, loading } = useIdeasStats();

  // Transform server-fetched posts (memoized, runs once)
  const initialIdeas = useMemo(() => initialPosts.map(transformIdea), [initialPosts]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            <IdeasList options={{ status: apiStatus, sort: sort as 'newest' | 'trending' | 'most_support', tags, searchQuery }} initialIdeas={initialIdeas} />
          </div>
