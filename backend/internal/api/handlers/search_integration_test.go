...
func (m *MockSearchRepositoryWithPosts) Search(ctx context.Context, query string, opts models.SearchOptions) ([]models.SearchResult, int, string, *float64, error) {
	m.lastOpts = opts
	...
	if start >= len(filtered) {
		return []models.SearchResult{}, total, "fulltext", nil, nil
	}
	...
	return filtered[start:end], total, "fulltext", nil, nil
}
