...
describe('SearchPage', () => {
  ...
  describe('Search results list', () => {
    it('fetches and displays search results from /v1/search', async () => {
      ...
      await act(async () => { render(<SearchPage />); });
      await waitFor(() => {
        expect(mockApiGet).toHaveBeenCalledWith('/v1/search', expect.objectContaining({ q: 'async' }), expect.any(Object));
        expect(screen.getByText('Race condition in async/await with PostgreSQL')).toBeInTheDocument();
  ...
  describe('Pagination', () => {
    ...
    it('displays current page number', async () => {
      ...
      mockApiGet.mockResolvedValue({ ...mockSearchResults, meta: { ...mockSearchResults.meta, page: 2 } });
      await act(async () => { render(<SearchPage />); });
    ...
    it('clicking previous page updates URL', async () => {
      setupSearchParams({ q: 'async', page: '2' });
      mockApiGet.mockResolvedValue({ ...mockSearchResults, meta: { ...mockSearchResults.meta, page: 2, has_more: true } });
      const user = userEvent.setup();
      await act(async () => { render(<SearchPage />); });
      await waitFor(() => {
    ...
    it('disables prev on first page and next on last page', async () => {
      setupSearchParams({ q: 'async' });
      mockApiGet.mockResolvedValue({ ...mockSearchResults, meta: { ...mockSearchResults.meta, has_more: false } });
      await act(async () => { render(<SearchPage />); });
      await waitFor(() => {
  ...
  describe('Empty state', () => {
    ...
    it('shows no results message with query and suggestions', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText(/try different|try another|modify your search/i)).toBeInTheDocument();
      });
    });
  });
  ...
  describe('Error state', () => {
    ...
    it('retries fetch when retry button is clicked', async () => {
      setupSearchParams({ q: 'async' });
      mockApiGet.mockRejectedValueOnce(new Error('Network error')).mockResolvedValueOnce(mockSearchResults);
      const user = userEvent.setup();
      await act(async () => { render(<SearchPage />); });
