...
func TestSearchRepository_Search_RelevanceScore(t *testing.T) {
	...
	results, _, _, _, err := repo.Search(ctx, "PostgreSQL", models.SearchOptions{
		Sort:    "relevance",
		Page:    1,
		PerPage: 20,
	})
...
func TestSearchRepository_Search_Snippet(t *testing.T) {
	...
	results, _, _, _, err := repo.Search(ctx, "async error", models.SearchOptions{
		Page:    1,
		PerPage: 20,
	})
...
func TestSearchRepository_Search_Pagination(t *testing.T) {
	...
	results, total, _, _, err := repo.Search(ctx, uniqueTerm, models.SearchOptions{
		Page:    1,
		PerPage: 2,
	})
	...
	// Request page 2
	results2, _, _, _, err := repo.Search(ctx, uniqueTerm, models.SearchOptions{
		Page:    2,
		PerPage: 2,
	})
...
func TestSearchRepository_Search_PerformanceTarget(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			start := time.Now()
			_, _, _, _, err := repo.Search(ctx, tc.query, tc.opts)
...
func TestSearchRepository_Search_ExactTitleMatch(t *testing.T) {
	...
	_, total2, _, _, err := repo.Search(ctx, "race conditions", models.SearchOptions{
		Page:    1,
		PerPage: 20,
	})
	...
	results3, _, _, _, err := repo.Search(ctx, "Race Conditions in Go", models.SearchOptions{
		Sort:    "relevance",
		Page:    1,
		PerPage: 20,
	})
...
func TestSearchRepository_Search_PartialWordMatch(t *testing.T) {
	...
	_, total1, _, _, err := repo.Search(ctx, "rac", models.SearchOptions{
		Page:    1,
		PerPage: 20,
	})
	...
	_, total2, _, _, err := repo.Search(ctx, "cond", models.SearchOptions{
		Page:    1,
		PerPage: 20,
	})
