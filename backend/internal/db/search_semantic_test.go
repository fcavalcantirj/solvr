...
func TestSearchHybrid_SemanticSimilarity(t *testing.T) {
	...
	results, total, _, _, err := repo.Search(ctx, "golang race condition", models.SearchOptions{
		Page:    1,
		PerPage: 20,
	})
...
func TestSearchHybrid_FallbackToFullText(t *testing.T) {
	...
	results, total, _, _, err := repo.Search(ctx, "golang", models.SearchOptions{
		Page:    1,
		PerPage: 20,
	})
...
func TestSearchHybrid_EmptyEmbeddings(t *testing.T) {
	...
	results, total, _, _, err := repo.Search(ctx, "null embeddings hybrid search", models.SearchOptions{
		Page:    1,
		PerPage: 20,
	})
...
func TestSearchHybrid_RRFWeighting(t *testing.T) {
	...
	results, total, _, _, err := repo.Search(ctx, "improve database query performance", models.SearchOptions{
		Page:    1,
		PerPage: 20,
	})
